...
(def hdr-color [:dark:bg-gray-800 :bg-gray-100 :xdark:bg-opacity-50])
...
(def std-selected-tab
  ["bg-black/25" :text-white "dark:bg-black/50"])
...
(def color-map
  ...
  {:list      {:icon     :compass
               ...
               :selected {:fg  [:text-alt-100]
                          :bg  (concat
                                 ["bg-alt-500" "dark:bg-alt-600" :text-white])
               ...
               :content  {:h2 [;:tracking-wide
                               ...
                               :uppercase]
                          :fg [:text-white]}}
   :bar-chart {:icon     :clock
               :normal   {:fg  [:text-pink-400]
                          :bg  []
                          :tab std-not-selected-tab}
   ...
   :brygge    {:icon     :brygge
               ...
               :selected {:fg  [:text-white]
                          ...
                          :tab std-selected-tab}}
   :cross-out {:icon :double-chevron-right}
