          ...
          [:div.p-4.space-y-4
           ...
           [:div.flex.justify-end.gap-4
            ...
            (bu/danger-button {:type     :submit
                               ...
                               :on-click #(eykt.fsm-helpers/send :e.hide)} "Lagre")]]])])))
...
(defn modal-header
  ...
  (let [star [:div {:on-click #(toggle-favorite-fn)
    ...
    [:div.grid.gap-2.p-4.text-base.font-normal.w-full.bg-gray-500
...
(defn build-content [{:keys [buttons buttons-cb ok]}]
...
(comment
  (do
    (build-content {:buttons-cb (fn [id] (get {:ok     "Okay then"
                                               :cancel "Back out"} id))
                    :buttons    #{:ok :cancel}
                    :ok         #(js/alert "!")})))
