...
(defn large-switch [{:keys [tag caption get set]}]
  (r/with-let [get-value get #_(schpaa.state/listen tag)
               set-value set #_#(schpaa.state/toggle tag)]
    (let [on? @get-value]
      [sc/row-sc-g2
       [ui/switch-group
        [ui/switch
         {:checked   on?
          ...
          :class     [:overflow-hidden
                      :relative :inline-flex :flex-shrink-0 :border-2 :border-transparent :rounded-full :cursor-pointer
                      :transition-colors :ease-in-out :duration-200 :focus:outline-none :focus-visible:ring-2 :focus-visible:ring-white]
          :style     {:background (if on? "green" "black")
                      :box-shadow "var(--inner-shadow-1),var(--shadow-2)"
                      :color      "var(--gray-1)"
                      :height     "30px"
                      :width      "58px"}}
         ...
         [:span.pointer-events-none.inline-block.transform.ring-0.transition.ease-in-out.duration-300
          {:aria-hidden true
           :class       (if on? :translate-x-7 :translate-x-0)
           :style       {:transform     ""
                         :border-radius "var(--radius-round)"
                         :background    "var(--gray-1)"
                         :height        "26px"
