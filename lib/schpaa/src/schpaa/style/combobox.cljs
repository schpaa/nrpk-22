...
(def commands
  [{:id "0", :name "Registrer skade på utstyr" :icon [:> outline/PencilIcon]}
   ...
   {:id "16", :name "Gå til retningslinjer" :icon [:> outline/LightBulbIcon] :action #(rf/dispatch [:app/navigate-to [:r.booking.retningslinjer]])}
   ...
   {:id "20", :name "Aktivitetsliste" :icon [:> outline/DocumentIcon] :action #(rf/dispatch [:app/navigate-to [:r.aktivitetsliste]])}
              ...
              [ui/combobox-input
               {;:as :div
                :style         {:width          "25rem"
                                ...
                                :color          "var(--text1)"
                                :background     "var(--surface00)" #_"var(--white)"}
