...
(defn listitem [{:keys [notfound selected action active disabled icon] :as attr} value]
  [:div.gap-3.select-none.truncate
   {:class [:snap-start]
    :style {:display         :flex
...
(o/defstyled placeholder :div.w-full
  [:input
   {:font-family "Montserrat"
    :font-weight 500
    ...
    (let [selected @!selected
      ...
      [:div
       ...
       [ui/combobox {:value     (:id commands)
        ...
        [:div.relative
         [:div.relative.w-fullx
          [:div.flex.items-center.justify-between
           [placeholder [ui/combobox-input {:style         {:width          "100%"
                                                            :height         :4rem
                                                            :padding-inline "var(--size-4)"}
                                            :class         [:outline-none :focus:outline-none :truncate]
