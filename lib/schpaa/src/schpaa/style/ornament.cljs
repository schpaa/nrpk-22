...
(o/defstyled with-focus :div
  ...
  ["&:hover:not(.focused)"
   {:color            "var(--gray-9)"
    ...
    :-outline         "4px solid var(--yellow-5)"}])

(o/defstyled surface-ab :div
  [:& :p-2 with-focus :select-none
...
(o/defstyled icon-tiny :div
  ;:span.w-6.h-6.bg-white.relative
  [:& :w-4 :h-4
   #_{:position :relative
...
(o/defstyled row-sc-g2 :div
  [:&
   ;:m-0 :p-0
   {:gap         "0.5rem"
    ;:outline "2px solid rgba(0,255,0,0.2)"
    :display     "flex"
    :align-items "center"}])
...
(o/defstyled badge :button
  ...
  [:&.small {:font-size "var(--font-size-3)"
             :max-width "4rem"}]
  [:&.normal {:color      "var(--gray-7)"
              :background "var(--gray-0)"}]
  [:&.selected {;:outline "4px dashed var(--yellow-5)"
                :color      "var(--brand1-copy)"
                :background "var(--brand1)"}]
  [:&.highlighted {:outline    "2px solid var(--red-5)"
                   :color      "var(--pink-2)"
                   :background "var(--pink-9)"}]
  #_([{:keys [selected on-click]} ch]
     ^{:on-click on-click}
...
(o/defstyled badge-2 :span
  [:& small-rounded :font-oswald
   {:cursor         :pointer
    ...
    :background     "var(--gray-9)"                         ;"var(--text0-copy)"
    :color          "var(--gray-1)"                         ;"var(--text0)"
    :letter-spacing "var(--font-letterspacing-1)"}
   ...
   [:&.small {:border-radius  "var(--radius-0)"
              ;:padding-inline "var(--size-2)"
              :width          "3em"                         ;var(--size-8)
              :min-width      "2em"
              :letter-spacing "var(--font-letterspacing-2)"
              :font-size      "var(--font-size-0)"}]
   [:&.out-of-order {:background "var(--red-7)"
                     :color      "var(--red-1)"}]
...
(defn mine-vakter-collapsable [tag header content & links]
  [ingress-details'
   {:class [:-debug2]
    :open  (if tag @(schpaa.state/listen tag) 1)
    :style {:margin-left "calc(var(--size-2) * -1)"
