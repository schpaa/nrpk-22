(ns schpaa.style.dialog
  (:require [headlessui-reagent.core :as ui]
            ...
            [schpaa.style.input :as sci :refer [input textarea]]))
  ...
  (fn [{:keys [form-id] :as props}]
    [:form
     ...
     [sc/col-fields {:class [:space-y-2]}
      [sc/col {:class [:px-2]}
       [sc/title-p "Header title"]
       [sc/text "Lang ledetekst som tvinger vinduet til å legge seg ut så bredt det kan"]]
      [sc/row-wrap
      ...
      [textarea props :text [] "Innhold" :stuff3]]]))

(defn addpost-form [{:keys [data type] :as context}]
  [fork/form {:initial-values  {:stuff  "flaff"
                                :stuff3 "# DetteA\n> Her en ingress\nSå kommer selve teksten\n### Kanskje med en overskrift\nHvem vet?"}
           ...
           [:div.fixed.inset-x-1
            ...
            [:div.text-center
             ...
             [ui/transition-child
              ...
              [sc/dialog {:class [:overflow-y-auto :max-h-screen]}
               (when auto-dismiss
               ...
               [fork/form {:keywordize-keys  true
                ...
                (fn [{:keys [form-id handle-submit] :as props}]
                  [:form.space-y-4.min-w-xs
                   ...
                   (when buttons
                     [:div.pt-4
                      [sc/row-end {:class [:justify-end :gap-2]}
                       (->> buttons
                            (map (fn [[kind caption action]]
                                   (cond
                                     (= kind :danger)
                                     [scb2/danger-large {:type "submit"} caption]

                                     (= kind :cta)
                                     [scb2/cta-large {:type     "submit"
                                                      :on-click #(do
                                                                   (reset! write-success true)
                                                                   (close))} caption]

                                     :else
                                     [scb2/normal-large {:type     "button"
                                                         :on-click #(do
...
(defn open-selector []
  (rf/dispatch [:lab/modal-selector
                true
                {:content-fn (fn [c]
                               [schpaa.style.combobox/combobox-example])}]))
        ...
        [:<>
         [ui/transition
          {:appear      true
           ...
           :show        open?}
          [ui/dialog {
                      :on-close #(if click-overlay-to-dismiss (close))}
           [:div.fixed.inset-x-1
            ...
            [:div.text-center
             [ui/transition-child
              {:enter      "ease-out duration-300"
               ...
               :leave-from "opacity-100"
               :leave-to   "opacity-0"}
              [ui/dialog-overlay {:style {:background "var(--surface1)"}
                                  :class " fixed inset-0 opacity-60"}]]
             ...
             [ui/transition-child
              {:class       "inline-block align-middlex text-left transform"
               :enter       "ease-out duration-200"
               :enter-from  "xopacity-0  -translate-y-4"
               :enter-to    "xopacity-100  translate-y-0"
               :leave       "ease-in duration-200"
               :leave-from  "opacity-100  translate-y-0"
              ...
              [sc/selector {:style {:box-shadow "var(--shadow-6), var(--shadow-6), var(--shadow-6)"}
                            :class [:xoverflow-y-auto :xmax-h-screen :-translate-y-24x :rounded-xl :overflow-hidden]}
               (if content-fn
                 (content-fn context)
                 [:div "Forgot content-fn?"])]]]]]]]))))