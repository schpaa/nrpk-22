...
(defn disp [{:keys [data on-close on-star-click star-count you-starred]}]
  (letfn [(f [stability expert]
    ...
    (let [{:keys [boat-type location slot material stability expert number navn kind description last-update weight length width aquired-year aquired-price]} data]
      [sc/dropdown-dialog
       [sc/col-space-8
        [sc/col-space-4
         [sc/row-sc-g2
          ...
          (when star-count
            [sc/text1 star-count])
          [sc/icon-large {:on-click on-star-click
                          :style    {:color "var(--yellow-6)"}} (if you-starred ico/stjerne ico/ikkeStjerne)]]
