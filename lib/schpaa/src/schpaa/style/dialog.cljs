...
(defn modal-example [vis]
  (r/with-let [!open? (r/atom vis)
               open #(reset! !open? true)
               close #(reset! !open? false)]
    (let [open? @!open?]
      [:<>
       [:div.flex                                           ;.items-center.justify-center
        [scb/cta                                            ;:button.px-4.py-2.text-sm.font-medium.text-white.bg-black.rounded-md.bg-opacity-20.hover:bg-opacity-30.focus:outline-none.focus-visible:ring-2.focus-visible:ring-white.focus-visible:ring-opacity-75
         {:type     "button"
          :on-click open}
         "Open dialog"]]
             ...
             [fork/form {:keywordize-keys  true
              ...
              (fn [{:keys [form-id handle-submit errors] :as props}]
                [:form.space-y-4
                 ...
                 [:div.py-2x.space-y-4 {:class [:x-debug]}
...
(defn modal-example-with-timeout [content]
  (r/with-let [!open? (r/atom false)
    ...
    (let [open? @!open?]
      [:<>
       [:div.flex                                           ;.items-center.justify-center
        [scb/normal                                         ;:button.px-4.py-2.text-sm.font-medium.text-white.bg-black.rounded-md.bg-opacity-20.hover:bg-opacity-30.focus:outline-none.focus-visible:ring-2.focus-visible:ring-white.focus-visible:ring-opacity-75
         {:type     "button"
          :on-click open}
         "Open dialog-2"]]
         ...
         [:div.fixed.inset-0.overflow-y-autox
          ...
          [:div.px-4.text-center
           ...
           [ui/transition-child
            ...
            [sc/dialog
             ;[:div @counter]
             [:svg.-mx-4.-mt-4 {:style   {:height :4px
                                          :width  "100%"}
