...
(defn disp [{:keys [data on-close on-star-click uid]}]
  (letfn [(f [stability expert]
    ...
    (let [{:keys [boat-type star-count location slot material stability expert number navn kind description last-update weight length width aquired-year aquired-price]} data]
         ...
         [sc/col-space-8
          [sc/col-space-4
           [sc/row-sc-g2
            ...
            (when (pos? @bt-data)
              [sc/text1 @bt-data])
            [sc/icon-large {:on-click #(on-star-click boat-type (not @ex-data))
                            :style    {:color (if @ex-data "var(--yellow-6)" "var(--text2)")}} (if @ex-data ico/stjerne ico/ikkeStjerne)]]
          ...
          [schpaa.style.hoc.page-controlpanel/togglepanel :boats/details "Tilsvarende"
           (fn []
             [sc/col-space-4
...
(defn open-modal-boatinfo [{:keys [uid data]}]
  (let [boat-type (some-> data :boat-type)
        #_#_starred-by-user? (some-> user :starred (get (keyword boat-type)))]
    #_(tap> [boat-type starred-by-user? (some-> user :starred (get (keyword boat-type)))])
