  ...
  [sc/dropdown-dialog
   [sc/col {:class [:space-y-8]}
    [sc/col {:class [:space-y-2 :w-full]}
     [sc/dialog-title "Slette konto"]
     [sc/subtext-p "Kontoen din blir markert som slettet og du vil bli logget ut på alle enheter. Etter 14 dager slettes alle data relatert deg."]
     [sc/text-p "Er du sikker på at du vil slette kontoen din?"]]
    [sc/row-ec
     [scb2/normal-regular {:type     "button"
                           :on-click #(on-close)} "Nei"]
     [scb2/cta-regular {:type     "button"
                        :on-click #(on-save)} "Slett i vei"]]]])
            ...
            [:div.text-center
             [ui/transition-child
              {:enter      "ease-out duration-300"
               ...
               :leave-to   "opacity-0"}
              [ui/dialog-overlay {:style {:background "var(--surface4)"
                                          :opacity    0.75}
                                  :class "fixed inset-0 "}]]
