...
(defn addpost-form [{:keys [data type on-close on-save] :as context}]
  [fork/form {:initial-values    {:title        ""
   ...
   (fn [{:keys [form-id handle-submit dirty] :as props}]
     (let [changed? dirty]
       [:form
        {:id        form-id
         :on-submit handle-submit}
        [sc/col-fields {:class [:space-y-2 :p-4]}
         [sc/col {:class [:px-2 :items-start]}
          [sc/title {:style {:color "var(--surface3)"}} "Poste nytt innlegg"]]
         [sc/row-wrap
          [input props :text [:w-min] "Tittel" :title]
        ...
        [:div.px-4.pb-4                                     ;.-mx-4
         ...
         [sc/row-end {:style {:padding-top "var(--size-3)"
                              :gap         "var(--size-3)"}}
          [scb2/outline-large {:type     "button"
                               :on-click #(on-close)} "Avbryt"]
          [scb2/cta-large {:disabled (not changed?)
                           :type     "submit"
            ...
            [:div.text-center
             ...
             [ui/transition-child
              ...
              [sc/dialog {:class [:overflow-y-auto :max-h-screen]}
               ...
               #_[fork/form {:keywordize-keys  true
                             :prevent-default? true
                             :initial-values   {}
                             :on-submit        (fn [v]
                                                 (reset! write-success true)
                  ...
                  (fn [{:keys [form-id handle-submit] :as props}]
                    [:form.space-y-4.min-w-xs
                     ...
                     (when buttons
                       [:div.pt-4
                        [sc/row-end {:class [:justify-end :gap-4]}
                         (->> buttons
