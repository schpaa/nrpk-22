...
(defn signin-content [{:keys [on-close on-save] :as context}]
  [sc/dropdown-dialog
   [:div.pb-8
    [sc/col-space-2
     [sc/title1 "Velkommen"]
     [sc/text1 "Velg hvilken identitets-tilbyder du vil bruke."]]
    ...
    [sc/col-space-2
     ...
     [sc/text1 "Når du logger inn kan vi tilby deg noe mer funksjonalitet."]
...
(defn standard-close-button [close]
  [:div.absolute.top-2.right-2.p-2
            ...
            [:div.text-center
             ...
             [ui/transition-child
              {:class       "inline-block align-middlex text-left transform"
               ...
               :entered     "drop-shadow-xl"
               :enter-from  "opacity-0  -translate-y-full"
               :enter-to    "opacity-100  -translate-y-2"
               :leave       "ease-in duration-300"
              ...
              [:div {:style {:width     "512px"
                             :max-width "95vw"}
                     :class [:overflow-y-auto :max-h-screen :min-w-xs :max-w-md]}
...
(defn command-palette [{:keys [context vis close]}]
  (let [{:keys [action on-primary-action buttons click-overlay-to-dismiss content-fn auto-dismiss]
         :or   {click-overlay-to-dismiss true}} context]
    (let [open? @vis]
      [ui/transition
         ...
         [:div.text-center
          ...
          [ui/transition-child
           {:class      ["inline-block align-middle text-left transform overflow-hidden "
            ...
            :style      {:border        "2px solid var(--brand1)"
            ...
            [:div.text-center
             ...
             [ui/transition-child
              {:style       {:border        "2px solid var(--brand1)"
               ...
               :enter       "ease-out duration-100"
               :enter-from  "xopacity-0  translate-y-16"
