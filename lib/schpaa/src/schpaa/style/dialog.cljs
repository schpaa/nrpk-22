...
(defn modal-confirm-delete [{:keys [content-fn context vis close action on-close]}]
  (r/with-let [write-success (r/atom false)]
    (let [open? @vis]
      [:<>
       [ui/transition
...
(defn modal-regular [{:keys [content-fn context show close-fn action on-close]}]
  (r/with-let [write-success (r/atom false)]
    (let [open? @show]
      [:<>
       [ui/transition
        {:appear true
         :show   open?}
        [ui/dialog {:on-close #(close-fn)}                  ;must press cancel to dismiss
         [:div.fixed.inset-x-1
          [:div.text-center
           [ui/transition-child
            {:enter      "ease-out duration-300"
...
(defn modal-with-timeout [{:keys [!open? close-fn context content-fn]}]
  (r/with-let [tm (r/atom nil)
               counter (r/atom 0)
               close #(do
                        (js/clearInterval @tm)
                        (close-fn))
            ...
            [sc/dialog
             [:svg.-ml-4.-mt-4.absolute {:style   {:width  :4px
              ...
              [:line {:stroke       "var(--brand1)"
               ...
               [:animate {:fill          "freeze"
                          :attributeName "y2" :from "50" :to "0" :dur "6s"}]]]
             [:div.w-full.space-y-4 (content-fn context)]
             [:div.mt-4 [sc/row {:class [:justify-end]}
                         [scb/normal
                          {:type     "button"
                           :on-click #(do
