    ...
    [sc/col {:class [:space-y-4]}
     ...
     [schpaa.style.switch/large-switch
      {:tag     :lab/toggle-chrome
       :caption [sc/subtext "Vis ekstra krom på kanten (ekstra knapper)"]}]

     [schpaa.style.switch/large-switch
      {:tag     :lab/hide-image-carousell
       :caption [sc/subtext "Skjul billed-karusell på forsiden"]}]
      ...
      (let [changed? dirty]
        [:form
         ...
         [sc/col-space-2
          ...
          [sc/col-space-1
           [sc/text1 (:navn values)]
           [sc/text1 (:uid values)]]

          [:div (interpose [:div.py-6]
