...
(defn dateinput [{:keys [cursor class] :as attr} label field]
  (cond
    cursor
    (sci/input attr (:type attr :date) {:class (conj class :on-bright :w-32)} label field)
    :else
    (sci/input attr (:type attr :date) {:class (conj class :on-bright :w-40)} label field)))

(defn timeinput [{:keys [cursor class] :as attr} label field]
  (cond
    cursor
    (sci/input attr (:type attr :time) {:class (conj class :on-bright :w-32)} label field)
    :else
...
(defn icon-and-caption
  ...
  ([attr icon caption]
   (let [large? (some #{:large} (:class attr))]
     (pill
       (merge-with into
                   {:style {:display     "flex"
                            :flex-shrink 0
                            :line-height 1}
                    :class (concat
                             (if icon [:pad-right] [:pad-right :pad-left])
