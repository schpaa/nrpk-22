...
(o/defstyled menu :div
  :h-12 :flex :gap-4 :items-center :px-4
  {:min-width   "24ch"
   :cursor      :pointer
   :color       "var(--text1)"
   :font-family "Inter"
...
(defn mainmenu-item-mm [{:keys [active disabled icon label shortcut]}]
  [menu
   {:class [(cond active :active
                  disabled :disabled :else nil)]}
   (if icon icon [:div.w-7.h-7])
   [:div.flex.justify-between.w-full.gap-4
    [(if disabled sc/text3 sc/text1) {:style {:white-space :nowrap}} label]
    (when shortcut
         ...
         [ui/transition
          ...
          [ui/menu-items
           ...
           [mainmenu-items-rounded-very
            {:style (cond
                      (some #{:down} dir) {:margin-top "-48px"}
                      :else {:bottom "48px"})
             :class [:space-y-1x :xml-32]}
            (doall (for [[type {:keys [disabled action content shortcut] :as args}] data]
                     (case type
                       ...
                       :header [ui/menu-item
                                {:as       "div"
                                 :class    [:outline-none]
                                 :on-click action #_#(rf/dispatch [:lab/close-menu])}
                                (fn [{:keys [active disabled]}]
                                  [custom-thang
                                   {:class [:px-3]}
                                   [sc/icon {:class [(if active :hovers :others)]} (:icon args)]
                                   [sc/as-shortcut shortcut]])]
