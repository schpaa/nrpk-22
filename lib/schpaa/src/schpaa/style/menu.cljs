...
(o/defstyled settings-menu :div
  [:&
   ...
   {:xposition     :absolute
    :z-index       100
    ;:min-width     "min(calc(100% - 2rem),32ch)"
    :width         "36ch"
    ...
    :background    "var(--toolbar)"
    :border-radius "var(--radius-3)"
    :box-shadow    "var(--shadow-6),var(--shadow-1)"}])
...
(defn mainmenu-item-mm [{:keys [tall active disabled icon label shortcut style]}]
  [menu
   {:style (when tall {:height "var(--size-9)"})
    :class [;(if tall :pt-16)
            (cond active :active
    ...
    (fn [{:keys [open]}]
      [:<>
       [ui/menu-button
        ...
        (button open)]
       [ui/transition
        {:class      (str "drop-shadow-sm " (if (some #{:right} dir) "-ml-2" "-mr-2"))
...
(defn build-menu [data]
  (into [settings-menu
         {:style {:xleft      :50%
          ...
          :class [:-debug]}]
        (for [[idx [type {:keys [disabled action content shortcut] :as args}]] (map-indexed vector data)
              :let [z (zero? idx)]]
...
(defn settings-floating [{:keys [button data showing!]}]
  #_[:div.select-none.z-50
     {:class [:fixeds :text-left :drop-shadow-xl]}
     #_[:div.select-none.fixed.inset-0.duration-500
        {:on-click #(button true)
         :class    [(if @showing! :pointer-events-auto :pointer-events-none)]
         :style    {:z-index          110
                    :background-color (if @showing! :#0005 :#0000)}}]]
  [ui/menu
   ...
   (fn [{:keys [open]}]
     (let [show @showing!]
       [:div
        ...
        [ui/menu-button
         {:as    "button"
          :class [(o/classname hoc.buttons/round') :x-debug3]}
         [button open]]

        [ui/transition
         {;:show       show
          :class      "drop-shadow-sm text-center"
          :xstyle     {:position      :fixed
                       ;:inset 0
                       ;:display :grid
          ...
          :enter      "ease-in-out transform duration-500 translate-x-0"
          :enter-from "opacity-0 translate-x-0 translate-y-0 scale-75" ;"opacity-50  scale-75"
          :enter-to   "opacity-100 translate-y-4 scale-100" ;"opacity-100  scale-100"
          :entered    "z-100 translate-y-0 translate-x-0"
          :leave      "ease-in-out transform duration-500"
          :leave-from "opacity-100 translate-y-4"
          :leave-to   "opacity-0 translate-y-32"}
         [ui/menu-items
          {:unmount true
           :class   [:focus:outline-none :absolute :inset-0 :w-auto]
           :style   {:left      :50%
                     :top       :50%
                     :transform "translate(-50%,-50%)"}}
          [build-menu data]]]]))])
