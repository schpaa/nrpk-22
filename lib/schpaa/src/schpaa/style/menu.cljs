...
(o/defstyled mainmenu-items-rounded :div
  [:& :p-4 :absolute :w-56 :focus:outline-none :outline-none
   {:z-index       11000
     ...
     [ui/transition-child
      ...
      [ui/menu-items
       ...
       [mainmenu-items-rounded
        {:class [:x-debug :space-y-1 :right-0]}
    ...
    [:div.select-none
     [ui/menu {:as :div.relative.inline-block}
      ...
      [ui/transition
       ...
       #_{;:show       true                                      ;always-show #_(or always-show false); true ;always-show
          :enter      "transition ease-out duration-100"
          :enter-from "opacity-0 "
          :enter-to   "opacity-100"
          :leave      "transition ease-in duration-300"
       ...
       [ui/menu-items
        ...
        [mainmenu-items-rounded {:class [(if up? :bottom-12 :top-12) :left-0]}
         ;[(if (= :down dir) menu-items-down menu-items-up)]
         (for [[type args] data]
           (case type
             :header [:div args]
