...
(defn mainmenu-item-mm [active disabled {:keys [icon label highlight] :as args}]
  [(cond
   ...
   icon [:div {:class [(if highlight :font-bold)]} label]])
     ...
     [ui/transition-child
      {
       ...
       :enter      "transition transition-transform ease-out duration-200 origin-top-right"
       :enter-from "opacity-0 -translate-y-32 rotate-12"
       ...
       [mainmenu-items-rounded
        ...
        [:div.absolute.top-2.right-2
         [ui/menu-button {:as :div} (fn [{:keys [open]}]
                                      (close-button open))]]
        (doall (for [[type {:keys [disabled] :as args}] data]
                 (case type
                   :header [:div args]
