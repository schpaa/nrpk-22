...
(defn mainmenu-item-mm [{:keys [action active disabled icon label highlight color] :as args}]
  [(cond
     highlight mainmenu-item-normal
     active mainmenu-item-active
     disabled mainmenu-item-disabled
     :else mainmenu-item-normal) (when (or highlight active)
                                   {:on-click #(action)
   ...
   (if icon
     ...
     [:div.w-8.h-8])
   [:div {
          :style (conj {;:outline "solid red 1px"
                        :xwidth :32rem}

                       (if disabled {:color "var(--surface1)"}
                                    {:color "var(--surface4)"}))
