...
(o/defstyled mainmenu-items-rounded-very :div
  [:& :p-1x :absolute :focus:outline-none :outline-none :overflow-hidden
   :min-w-max
  ...
  #_[(cond
     ...
     [sc/row' {:style {:color (if disabled "var(--surface1)" "var(--surface4)")}
      ...
      [sc/text1-cl {:style {:font-family "Inter"
                            ...
                            :color       (if disabled "var(--text3)" "var(--text1)")}} label]
      (when shortcut
        [sc/as-shortcut shortcut])]])
  ...
  (let [up? (some #{:up} dir)
    ...
    [:div.select-none
     ...
     [ui/menu {:as :div}
      [ui/menu-button {:as :div}
       (fn [{:keys [open]}]
         (when button [:div.mb-1x (button open)]))]
      [ui/transition
       {:class      ""
       ...
       [ui/menu-items
        ...
        [mainmenu-items-rounded-very
         {:class [:space-y-1
                  #_#_(if up? :bottom-12 :top-20
                          (if right? :-right-1 :left-12))]}
         #_[:div.absolute.top-1.left-1
            [ui/menu-button {:as :div} (fn [{:keys [open]}]
                                         (close-button open))]]
         (doall (for [[type {:keys [disabled action color content] :as args}] data]
                  (case type
                    :header [:div args]
                    :toggle [ui/menu-item
                             {:disabled disabled
