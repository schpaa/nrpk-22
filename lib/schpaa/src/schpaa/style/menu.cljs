...
(defn standard-menu-animations [dir]
  (let [up? (some #{:up} dir)
        ...
        dur "duration-150 transform-gpu"
        enter-from-translate (if up? "translate-y-12" "-translate-y-32")
        enter-to-translate (if up? "translate-y-2" "-translate-y-10")
...
(defn mainmenu-item-mm [active disabled {:keys [icon label highlight color] :as args}]
  [(cond
...
(defn smaller-menuitem [active disabled {:keys [icon label highlight]}]
  [(cond
   ...
   {:class [:justify-start :items-center :gap-2 :h-8 :px-2]}
   [sc/icon icon]
   [:div {;:on-click #(do (tap> "CLICK"))
          :style (if disabled {:color "var(--surface1)"}
                              {:color "var(--surface4)"})
                    ...
                    :menuitem [ui/menu-item
                               ...
                               (fn [{:keys [active disabled]}]
                                 [:div.focus:outline-none {:on-click action}
                                  [mainmenu-item-mm active disabled args color]])]
                    :menuitem- [ui/menu-item
                                {:disabled disabled}
                                (fn [{:keys [active disabled]}]
                                  [:div.focus:outline-none {:on-click action}
                                   [smaller-menuitem active disabled args color]])]
                    :hr [:div.py-2 [hr]]
                    :space [:div.py-1]
                    :footer [:div args]
                    :div args
...
(defn naked-menu-example-with-args [{:keys [button dir data showing] :or {showing false}}]
  (let [up? (some #{:up} dir)
        right? (some #{:right} dir)]
    [:div.select-none
     [ui/menu {:as :div.relative.inline-block}
      ...
      [ui/transition
       (conj
         {:show (or false showing)}
         (standard-menu-animations dir))
       [ui/menu-items
        {:static (not showing)
         :class  [:focus:outline-none]}
        [mainmenu-items-rounded-very {:class [(if up? :bottom-12 :top-8)
