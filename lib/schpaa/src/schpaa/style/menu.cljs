...
(o/defstyled menu :div
  [:& :flex :gap-4 :items-center :px-4
   ...
   [:&.taller {:padding-block "var(--size-3)"}]
   [:&.active
    {:background "var(--content)"}]
   [:&.disabled
    {:cursor :default}]])

(defn mainmenu-item-mm [{:keys [tall active disabled icon label shortcut style class]}]
  [menu
   {:style (when tall {:min-height "var(--size-9)"})
    :class [class
            (cond active :active
...
(defn smaller-menuitem [{:keys [active disabled icon label highlight]}]
  [:div {:style {:max-width "13rem"}}
   [(cond
    ...
    [sc/icon icon]
    [sc/subtext {:style (if disabled {:color "var(--surface1)"}
                                     {:color "var(--surface4)"})
...
(defn settings-floating [{:keys [data]}]
  [ui/menu
