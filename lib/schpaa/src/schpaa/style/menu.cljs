...
(defn settings-floating [{:keys [button data showing! dir]}]
  [:div.select-none.z-50.w-full
   {:class [:drop-shadow-xl]}
   [:div.select-none.fixed.inset-0.bg-transparent.duration-500
    {:on-click #(button true)
     :class    [(if @showing! :pointer-events-auto :pointer-events-none)]
      ...
      [:<>
       ...
       [ui/transition
        {;:show true
         ...
         :enter-from "opacity-50 translate-y-full"          ;"opacity-50  scale-75"
         :enter-to   "opacity-100 translate-y-32"           ;"opacity-100  scale-100"
         :entered    "z-100 "
         :leave      "ease-out duration-200"
