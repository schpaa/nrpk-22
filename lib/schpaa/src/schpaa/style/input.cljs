...
(defn input [{:keys [on-key-down errors values handle-change placeholder] :as props} type attr label field-name]
  (let [field-name (if field-name field-name #())
        has-errors? (field-name errors)]
    [sc/label-field-col
     {:style {:column-gap (if label "2rem" 0)}
      :class (conj (:class attr) [:-debug :gap-y-0])}
     (when label
       [sc/row {:class [:gap-1]}
        [sc/label (or label "label")]
        (when errors
          (if (field-name errors)
            [sc/label-error (first (field-name errors))]))])
      ...
      [:div
       ...
       [ui/combobox {:value     (:id selected)
                     :on-change #(do
                                   ...
                                   (on-change %))
                     :class     [:outline-none]}
        [:div.relative.mt-1x.h-12
         [:div.relative.mt-1x.w-full.text-left.cursor-default.h-full.outline-none
          {:class []}
          [combobox''
           [ui/combobox-input {
                               :class         [:pl-3 :pr-10 :h-10
                                               ;:outline-none
                                               (some-> (schpaa.style.button2/focus-button) last :class first)]
                               :style         {:border-radius "var(--radius-1)"
                                               :color         "var(--fieldcopy)"
