...
(defn general-combobox [{:keys [name] :as props}
                        {:keys [has-errors? disabled class on-new on-change value value-by-id items]}]
  ...
  (r/with-let [selected* (r/atom (value-by-id value))
    ...
    (let [selected @selected*
      ...
      [:div
       {:class class}
       ;[l/pre-s selected]
       [ui/combobox {:value     (:id selected :new)
                     :nullable  true
                     :name      name
                     :disabled  disabled
                     :on-change (fn [id]
                                  (let [q @query*]
                                    (if (nil? id)
                                      ;(tap> ["zzzCREATE NEW ITEM"])
                                      (do
                                        ;(tap> ["on-change(new)" id q])
                                        ;her er det nødt å skje
                                        (when on-new
                                          (reset! selected* nil)
                                          (tap> ["selected" selected "id" id "q" q])
                                          (on-new q)
                                          #_(tap> (count items))
                                          #_(when on-change
                                              (on-change :new q))))
           ...
           [:div.relative
            [:div.relative.w-full.text-left.cursor-default.h-full.outline-none
              ...
              [ui/combobox-input {:class         [:pl-3 :pr-7 :h-10 :w-full
                                  ...
                                  :display-value #(when %
                                                    ...
                                                    (str (:name (if (= % "new")
                                                                  {:name @query*}
                                                                  (value-by-id %)))))
                                  :on-change     #(let [new-value (string->query (.. % -target -value))]
                                                    (tap> ["on-change(1)" new-value])
             ...
             [ui/combobox-button {:style {:color "var(--text2)"}
              ...
              [sc/icon-small {:aria-hidden true} ico/comboBoxSelector]]]
            ...
            [ui/transition
             ...
             [ui/combobox-options
              ...
              (fn [{:keys [open]}]
                (into [:<>]
                      (concat
                        (when (empty? @filtered-items*)
                          [(sample-item {:id nil :name @query*} "Lag ")])
...
(defn combobox [{:keys [errors touched set-values values] :as props}
                {:keys [on-add items sidecar-right class label fieldname]}]
  {:pre [(vector? class)]}
  (r/with-let [items (r/atom (if items items [{:id 1 :name "Sample"}]))]
    (let [touched (or touched (fn [_] false))
          has-errors? (and (touched fieldname)
                           (get errors fieldname))]
      [sc/label-field-col {:class [:w-full]}
       ;[l/pre-s touched (get errors fieldname) fieldname (touched fieldname)]
       [l/pre-s
        (filter (fn [{:keys [id]}] (= id :new)) @items)]
       ...
       [b/ro {:class [:w-full]}
        ...
        [general-combobox
         ...
         {:value-by-id #(get (zipmap (map :id @items) @items) (keyword %))
          ...
          :name        fieldname
          :on-new      (fn [name]
                         (set-values {fieldname name})
                         ;(on-add name)
                         #_(swap! items conj {:id "new" :name name}))

          :on-change   (fn [id name]
                         (tap> ["............." id name])
                         (if (= :new id)
                           (set-values {fieldname name})
                           (set-values {fieldname id})))
