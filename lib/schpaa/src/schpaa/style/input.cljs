...
(defn common-style [has-errors?]
  (conj
    {:background    (if has-errors? "var(--red-2)" "var(--surface000)")
       ...
       [ui/combobox {:value     (:id selected)
        ...
        [:div.relative.mt-1.h-12
         [:div.relative.mt-1.w-full.text-left.cursor-default.overflow-hiddenx.h-full.outline-none
          ...
          [ui/combobox-button {:class [:focus:outline-none
                                       :outline-none
                                       ":active:bg-white absolute inset-y-0 right-0 flex items-center pr-2 h-10 "]}
         ...
         [ui/transition
          ...
          [ui/combobox-options {;:static true
                                ...
                                :class (into class ["bg-white absolute py-1 mt-2 overflow-auto max-h-48 "])}
           [:<>
