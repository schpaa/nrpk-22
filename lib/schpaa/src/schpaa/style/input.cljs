...
(defn textarea [{:keys [errors values handle-change] :as props} type attr label field-name]
  (let [has-errors? (field-name errors)]
    [sc/col {:class (into [:gap-1 :h-auto] (:class attr))}
