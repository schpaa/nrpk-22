...
(defn input [{:keys [cursor on-key-down errors values handle-change placeholder] :as props}
             type attr label field-name]
...
(o/defstyled combobox'' :div
  [:& schpaa.style.button2/focus-button
   ...
   [:&.on-bright {:background "var(--content)"}]])
        ...
        [:div.relative
         [:div.relative.w-full.text-left.cursor-default.h-full.outline-none
          ...
          [ui/combobox-button {:style {:color "var(--text2)"}
                               :class [:focus:outline-none
                                       :outline-none
                                       "absolute inset-y-0 right-0 flex items-center pr-2 h-8"]}
           [sc/icon-small {:aria-hidden true} [:> solid/ChevronDownIcon]]]]
