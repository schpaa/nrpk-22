...
(defn general-combobox [{:keys [has-errors? disabled name class on-change value value-by-id items]}]
...
(defn combobox [{:keys [errors items touched sidecar-right] :as props} class label fieldname]
  {:pre [(vector? class)]}
  (let [touched (or touched (fn [_] false))
        has-errors? (and (touched fieldname)
                         (get errors fieldname))
    ...
    [l/boundary
     [sc/label-field-col {:class [ :w-full]}
      ...
      [b/ro {:class [:w-full]}
       [general-combobox
        (conj props
              {:value-by-id #(get (zipmap (map :id items) items) %)
               :name        fieldname
