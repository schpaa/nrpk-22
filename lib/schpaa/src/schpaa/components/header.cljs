   ...
   [:div.relative
    [:div.flex.items-center.justify-between.gap-2.px-4.h-16.select-none.xshadow-sm.relative
     ...
     [:div.absolute.left-0.flex
      [:div {:class    (if (= (current-page) :r.common) [:bg-white :text-red-500] ["text-red-500/50"])
             :on-click #(navigate-to-home)}
       [:div.p-4 [icon/touch :house]]]
      [:div {:class    (if (= (current-page) :r.user) [:bg-white :text-black] ["text-black/50"])
     ...
     (if (get-writingmode-fn)
       ...
       [:div.flex-grow
        ...
        [:div.text-center.text-sm.font-normal.tracking-wide {:class ["text-black/50"]} (current-page-title)]])
