    ...
    [:div.flex.flex.flex-center
     ...
     (if (or
       ...
       [:div.relative
        ...
        (when-not (user-auth)
          [:div.absolute.inset-0.animate-ping.text-alt {:on-click #(navigate-to-user)}
...
(defn title [{:keys [current-page menu-direction current-page-title current-page-subtitle user-auth] :as m}]
  (let [{:keys [fg- fg fg+ p+ p- p]} (st/fbg' :header)
    ...
    (if menu-direction
      [:div.flex.items-center.mb-1x.antialiased
       {:class (concat
                 [:justify-start]
                 [:text-lg :antialiased :leading-snug :font-bold])}
       (let [title (current-page-title)
             [long-form short-form] (if (vector? title) title [title nil])
             sub-header (current-page-subtitle)]
         [:div.leading-normal.whitespace-nowrap
          {:class (concat [:text-left] fg p-)}
          [:div.py-0.flex.items-center.divide-dotted.divide-x-2.divide-gray-500
           {:class (concat fg p+)}
           [:div.pr-2 (home-icon current-page)]
           [:div.pl-2 sub-header " · " app-name]]
          [:div {:class (concat fg+ p-)} sub-text]])]
      [:div.flex.items-center.mb-1x.antialiased
       {:class (concat
                 [:justify-end]
                 [:text-lg :antialiased :leading-snug :font-bold])}
       (let [title (current-page-title)
         ...
         [:div.leading-normal.whitespace-nowrap
          {:class (concat [(if menu-direction :text-left :text-right)] fg p-)}
          [:div.py-0.flex.items-center.divide-dotted.divide-x-2.divide-gray-400
           {:class (concat fg p+)}
           [:div.pr-2 app-name " · " sub-header]
           [:div.pl-2 (home-icon current-page)]]
          [:div {:class (concat fg+ p-)} sub-text]])])))
