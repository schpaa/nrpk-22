  ...
  [:div.h-16.shadow-sm
   ...
   [:div.flex.items-center.justify-between.gap-2.px-4.h-16.select-none.relative
    ...
    [:div.absolute.left-0.flex
     ...
     #_[:div {:class    (if (some #{(current-page)} [:r.user :r.my-bookings]) [:bg-white :dark:bg-gray-600 :text-black :dark:text-gray-200] ["text-black/50" :dark:text-gray-400])
