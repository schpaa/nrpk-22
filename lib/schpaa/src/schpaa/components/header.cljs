  ...
  [:div.h-16.shadow-sm
   ...
   [:div.flex.items-center.justify-between.gap-2.px-4.h-16.select-none.relative
    ...
    [:div.absolute.left-0.flex
     [:div {:class    (if (= (current-page) :r.common) [:bg-white :dark:bg-gray-600 :text-red-500 :dark:text-red-500] ["text-red-500/50"])
            :on-click #(navigate-to-home)}
      [:div.p-4 [icon/touch :house]]]
     [:div {:class    (if (= (current-page) :r.user) [:bg-white :dark:bg-gray-600 :text-black :dark:text-gray-200] ["text-black/50" :dark:text-gray-400])
