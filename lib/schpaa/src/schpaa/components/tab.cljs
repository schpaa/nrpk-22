      ...
      {:display-name
       ...
       (fn [c]
         (let [_ (reset! node (reagent.dom/dom-node c))
           ...
           (doto @node
             ;(.addEventListener "resize" (fn [x] (tap> '(-> x .-target .-clientWidth))))
             ;(.addEventListener "scrollend" (fn [] (tap> "END")))
       ...
       (fn [{class      :class
         ...
         (let [on-class (or on-class [:btn-tab])
           ...
           [:div.sticky.top-16.flex.snap-x.snap-mandatory.overflow-x-auto.w-fullx.hide-scrollbar.bg-gray-200.h-12x
            {:style {:scrollbar-width :none}}

            (for [[page title action & {:keys [icon]}] m
                  :let [selected? (= selected page)]]
              [:div.shrink-0.flex.justify-center.snap-center.h-12
               ...
               [:button.justify-self-center.self-center.-debugx.rounded
                ...
                [:div.flex.gap-2.items-center.justify-center.w-full.px-1
                 {:class []}
                 (if icon [:div.w-5.h-5 [icon/adapt icon 1.2]])
                 [:div.text-base title]]]])]))})))
