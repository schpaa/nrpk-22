      ...
      {:display-name
       ...
       (fn [{locked?  :locked?
         ...
         (let [{:keys [bg p p+ fg fg+ hd br br+]} (st/fbg' :tabbar)
           ...
           [:div.flex.snap-x.snap-mandatory.overflow-x-auto.hide-scrollbar.z-50
            ...
            (for [[page title action & {:keys [icon]}] m
              ...
              [:div.shrink-0.flex.justify-center.snap-start.h-12
               ...
               [:button.justify-self-center.self-start.rounded.mt-3
                (conj {:class (concat [:outline-none :focus:outline-none :focus:ring-offset-transparent :focus:ring-0])}
                      (when-not locked?
                        {:on-click #(do (o page)
                                        (when (some? action)
                                          (action)))}))
                [:div.flex.gap-2.items-center.justify-center.w-full.whitespace-nowrap.overflow-clip
                 (if icon [:div.w-6.h-6 {:class fg+} [icon/adapt icon 1.4]])
                 [:div
                  {:class p}
                  title]]]])]))})))
