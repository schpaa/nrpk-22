    ...
    (r/create-class
      {:display-name
       ...
       (fn [{class      :class
             ...
             selected   :selected
             select     :select}]
         (let [{:keys [bg p fg fg+ hd]} (st/fbg' :surface)
               on-class (concat hd bg fg+ [:border-b-4 :border-alt])
               off-class (concat hd bg fg)
               o (or select #(rf/dispatch [:app/navigate-to [%]]))]
           [:div.sticky.top-16.flex.snap-x.snap-mandatory.overflow-x-auto.hide-scrollbar.z-50
            {:class bg
                ...
                (conj {:class [:outline-none :focus:outline-none :focus:ring-offset-transparent :focus:ring-0]}
                      (when-not locked?
                        {:on-click #(do (o page)
                                        (when (some? action)
                                          (action)))}))
                [:div.flex.gap-4.items-center.justify-center.w-full
                 {:class p}
                 (if icon [:div.w-6.h-6 [icon/adapt icon 1.6]])
                 title]]])]))})))
