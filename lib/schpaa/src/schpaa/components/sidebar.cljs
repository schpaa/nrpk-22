...
(defn brygge-content []
  [:div.space-y-4.p-3
   ...
   [:div.space-y-1.flex.flex-col
    ...
    [:button.btn.btn-free
     {:on-click #(apply eykt.state/send
                        (msg/are-you-sure-forced?
                          {:on-confirm  (fn [] (js/alert "confirm"))
                           :on-close    (fn [] (eykt.state/send :e.hide))
                           :alternative "Ok"
                           :title       "Overskrift"
    ...
    [:button.btn.btn-danger
     {:on-click #(apply eykt.state/send
                        (msg/are-you-sure-cover-and-forced?
                          {:on-confirm  (fn [] (js/alert "confirm"))
                           :on-close    (fn [] (eykt.state/send :e.hide))
                           :alternative "Nei"
                           :primary     "Ja"
