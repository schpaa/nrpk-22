...
(defn label-colors [readonly?]
  (let [{:keys [fg- fg]} (st/fbg' :field-label)]
    (if readonly?
      fg-
      fg)))
...
(defn field-colors [readonly?]
  (let [{:keys [bg bg- fg br br- fg- fg+ hd p p- he]} (st/fbg' :field)]
    (if readonly?
      (concat fg- bg- br-
              [:border-2
...
(defn text [{:keys [naked? ref class touched errors handle-change handle-blur values readonly?]} & {:keys [label name]}]
...
(defn textarea [{:keys [naked? auto-focus readonly? ref placeholder touched errors handle-change handle-blur values]} & {:keys [label name error-type]}]
  (let [error? (and (touched name) (get errors name))]
    [:div.flex-col.flex
     (when-not naked? [:label {:class (label-colors readonly?)
                               :for   name} label])
     [:textarea
