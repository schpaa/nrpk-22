...
(defn date [{:keys [touched errors handle-change handle-blur values class readonly?]} label name]
  (let [error? (and touched (touched name) (get errors name))]
    [:div.flex-col.flex
     {:class class}
     [:label {:for name} label]
...
(defn time [{:keys [naked? touched errors handle-change handle-blur values class]} label name]
  (let [error? (and touched (touched name) (get errors name))]
    (if naked?
      [:input.h-full.xfield-w-max
       {:class     (concat [:form-input :rounded :border-none]
                           class
                           (when error? field-error-classes))
        :name      name
        :id        name
