...
(def kind-table
  {"re-kayak" "Rekreasjons\u00ADkajakk"
   "fvkayak" "Flattvanns\u00ADkajakk"
   "havkayak" "Havkajakk"
   "sup" "Standup padlebrett"
   "grkayak" "Grønnlands\u00adkajakk"
   "surfski" "Surfski"
   "kano" "Kano"
   "robåt" "Robåt"
   "sgkayak" "Skjærgårds\u00ADkajakk"})

(defn normalize-kind [kind]
  (get kind-table kind (str "?" kind "?")))
...
(defn number-view [n]
  [:div.font-oswald.tracking-wider.text-xl.font-semibold.leading-normal.h-full.w-10.flex.justify-center.items-center.rounded-sm.h-8.shrink-0
...
(defn slot-view [slot]
  [:div.flex.flex-center.rounded-sm.w-auto.h-5.px-1
   {:class ["dark:bg-gray-600"
            ...
            ;"bg-clear"
            "text-gray-700"]}
   [:div.font-oswald.font-normal.whitespace-nowrap.text-sm.tracking-wider (first slot) (last slot)]])
...
(defn modern-selectbox' [{:keys [get-details set-details disabled?]} label-fn]
  (let [value (get-details)
        checkbox [:button {:on-click #(set-details (mod (inc value) 3))
                           :disabled disabled?
                           :type     :button
                           :style    {:width 68}
                           :class    (concat
                  ...
                  [:span {:class (concat
                                   ...
                                   [:bg-white
                                    ...
                                    :rounded-full])}]]]
    [:div.flex.items-center.gap-4.select-none
     (when-not disabled? {:on-click #(set-details (mod (inc value) 3))})
     (label-fn checkbox)]))
