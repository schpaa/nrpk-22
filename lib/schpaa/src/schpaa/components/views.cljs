...
(defn name-view [name]
  [:div.font-semibold.text-base.truncate
   ;{:class ["dark:text-gray-100" :text-gray-700]}
   name])

(defn number-view [n]
  [:div.font-oswald.tracking-wider.text-xl.font-semibold.leading-normal.h-full.w-10.flex.justify-center.items-center;.transform.-rotate-3
...
(defn slot-view [slot]
  [:div.flex.items-center.justify-center.w-8.rounded-sm.h-auto.mb-px
...
(defn modern-checkbox' [{:keys [get-details set-details]} label-fn]
  (let [value (get-details)
        checkbox [:button {:on-click #(set-details (not value))
                           :type :button
                           :class    (concat
                                       (if value [:bg-green-600] [:bg-gray-200 :dark:bg-gray-800])
                                       [:relative
                                        :inner-shadow
                                        :bg-gray-300
                                        :inline-flex :shrink-0
                                        :focus:outline-none :focus:ring-2 :focus:ring-offset-2 :focus:ring-cyan-400
                                        :rounded-full :border-2 :dark:border-gray-900  :border-gray-600
                                        :transition-colors :ease-in-out :duration-100
                                        :h-7 :w-12])}
                  [:span {:class (concat
    ...
    [:div.flex.items-center.gap-4.select-none
     {:on-click #(set-details (not value))}
     (label-fn checkbox)]))
