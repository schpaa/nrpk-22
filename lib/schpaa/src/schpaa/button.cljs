...
(defn build-buttonbar-content [{:keys [buttons button-captions ok form-id]}]
  (cond-> [:div.flex.justify-end.gap-4]
    (:close buttons) (conj (regular-button {:on-click #(eykt.fsm-helpers/send :e.hide)} (or (and button-captions (button-captions :close)) "Ok")))
    (:submit buttons) (conj (regular-button {:type :submit :form form-id :on-click #(eykt.fsm-helpers/send :e.hide)} (or (and button-captions (button-captions :submit)) "Submit")))
    (:ok buttons) (conj (danger-button {:on-click #(do (ok) (eykt.fsm-helpers/send :e.hide))} (or (and button-captions (button-captions :ok)) "Ok")))
    (:cancel buttons) (conj (regular-button {:on-click #(eykt.fsm-helpers/send :e.hide)} (or (and button-captions (button-captions :cancel)) "Avbryt")))))