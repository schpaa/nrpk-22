...
(def color-map
  {:form    {:bgt [:bg-gray-300 :text-gray-700
   ...
   :message {:bgt [:bg-sky-500 :text-sky-50]
             ...
             :bgp [:bg-gray-300 :text-gray-700 :dark:bg-gray-500 :dark:text-gray-100]
...
(defn form [{:keys [header form-fn footer dialog-type flags] :as m}]
  (let [{:keys [bg bgp bgf bgt]} (get color-map dialog-type #_(cond
    ...
    [:div.relative.top-0
     ...
     (when footer
       [:div
        {:class (concat bgf [#_:h-12 :flex :items-center :justify-between :xpx-2])}
        [:div footer]])]))
