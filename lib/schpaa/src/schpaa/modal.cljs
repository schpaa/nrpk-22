...
(def color-map
  {:form    {:bgt [:bg-gray-100 :text-gray-700
                   "dark:bg-gray-600"]
             :bgp [:bg-gray-200 :text-gray-700
                   "dark:bg-gray-300"]
       ...
       (fn [{:keys [show? config-fn]}]
         (let [;todo Possibly move this into the reagent-render method
           ...
           [:div.fixed.inset-x-1.select-none.rounded-b-lg
            {:class (concat (if (some #{:cover} flags) [:max-w-smx :top-32 :rounded] [:top-0])
                            (if (some #{:wide} flags) [:max-w-smx] [:max-w-xsx])
