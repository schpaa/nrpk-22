...
(defn init! [{:keys [config ports]}]
  (let [{:keys [database firestore storage]} ports]
    (.log js/console "db/init!")
    (reset! db.state/app (initializeApp (clj->js config)))
    (when debug?
      (connectDatabaseEmulator (getDatabase) "localhost" database)
      (connectStorageEmulator (getStorage) "localhost" storage)
      (connectFirestoreEmulator (getFirestore) "localhost" firestore))))
