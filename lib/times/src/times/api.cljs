...
(defn duration->text
  ...
  ([event opts]
   (if (t/= event (t/date-time))
     "nå"
     (let [{:keys [rounded? weeks days hours minutes seconds]}
           (if (t/< event (t/date-time))
             (count-generic
...
(defn month-name [wd & {:keys [length]}]
  (let [month-names '[januar februar mars april mai juni juli august september oktober november desember ?]
    ...
    (if (some? length)
      (subs (nth month-names d) 0 length)
      (nth month-names d))))
...
(defn date-format [d]
  (format "%d. %s %d" (t/day-of-month d) (month-name d) (t/year)))

(defn short-date-format [d]
  (format "%d.%02d" (t/day-of-month d) (t/int (t/month d))))
