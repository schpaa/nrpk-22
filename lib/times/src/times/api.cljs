...
(defn calendar-date-format [d]
  (format "%s %d. %s"
          (day-name (t/int (t/day-of-week d)) :length 3)
          (t/day-of-month d)
          (month-name d)))

(defn arrival-date [d]
  (format "%s %d. %s kl. %s"
          (day-name (t/int (t/day-of-week d)))
          (t/day-of-month d)
          (month-name d)
          (time-format d)))

(defn compressed-date [d]
  (format "%s %d.%d kl. %s"
          (day-name (t/int (t/day-of-week d)) :length 3)
          (t/day-of-month d)
          (t/int (t/month d))
...
(defn time-format [wt]
  (format "%02d:%02d" (t/hour wt) (t/minute wt)))

(defn time-format-hour [wt]
  (format "%02d" (t/hour wt)))

(defn tech-date-format [d]
  (format "%04d-%02d-%02d" (t/year) (t/int (t/month d)) (t/day-of-month d)))

(defn date-format [d]
  (format "%s %d. %s %d" (day-name (t/int (t/day-of-week d))) (t/day-of-month d) (month-name d) (t/year d)))

(defn logg-date-format [d]
  (format "%s %d.%d"
          (day-name (t/int (t/day-of-week d)) :length 2)
          (t/day-of-month d)
          (t/int (t/month d))))
...
(defn day-month [d]
  (format "%d.%d." (t/day-of-month d) (t/int (t/month d))))

(defn short-date-format [d]
  (format "%s %d/%02d" (day-name d :length 3) (t/day-of-month d) (t/int (t/month d))))
